/******************************************************************************
 * Objective: Wrapper over functions needed in the API in order to be able to
 *            read and write stringified JSON
 * Author:    Kenneth Nierenhausen
 * Date:      July 11, 2014
 * ref https://github.com/nplus11/rapid-json-wrapper

 *****************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <stdbool.h>

/* Guard to make sure that this is used as C */
#ifdef __cplusplus
extern "C" {
#endif

/* A collection of function pointers needed by Reader in rapidJSON */
typedef struct tm_json_r_handler {
  bool (*Default)(void);
  bool(*Null)(void);
  bool(*Bool)(bool);
  bool(*Int)(int);
  bool(*Uint)(unsigned);
  bool(*Int64)(int64_t);
  bool(*Uint64)(uint64_t);
  bool(*Double)(double);
  bool(*String)(const char*,size_t,bool);
  bool(*StartObject)();
  bool(*EndObject)(size_t);
  bool(*StartArray)(void);
  bool(*EndArray)(size_t);
  bool(*Key)(const char *, size_t, bool);
  bool(*RawNumber)(const char *, size_t, bool);
} tm_json_r_handler_t;

/* Writer and StringBuffer for the JSON writing */
typedef void* tm_json_writer_t;
typedef void* tm_json_stringbuffer_t;

/* Contains pointers to Writer and String Buffer */
typedef struct tm_json_w_handler {
  tm_json_writer_t writer;
  tm_json_stringbuffer_t stringBuffer;
} tm_json_w_handler_t;

/* Reading prototypes */
bool tm_json_parse(tm_json_r_handler_t*,const char*);
bool tm_json_read_default(void);
bool tm_json_read_null(void);
bool tm_json_read_boolean(bool);
bool tm_json_read_int(int);
bool tm_json_read_uint(unsigned);
bool tm_json_read_int64(int64_t);
bool tm_json_read_uint64(uint64_t);
bool tm_json_read_double(double);
bool tm_json_read_string(const char*,size_t,bool);
bool tm_json_read_startObject(void);
bool tm_json_read_endObject(size_t);
bool tm_json_read_startArray(void);
bool tm_json_read_endArray(size_t);
bool tm_json_read_key(const char *, size_t, bool);
bool tm_json_read_rawnumber(const char *, size_t, bool);

/* Writing prototypes */
tm_json_w_handler_t tm_json_write_create();
int tm_json_write_key(tm_json_w_handler_t, const char *);
int tm_json_write_keyn(tm_json_w_handler_t, const char *, size_t);
int tm_json_write_string(tm_json_w_handler_t, const char*);
int tm_json_write_stringn(tm_json_w_handler_t, const char*,size_t);
int tm_json_write_boolean (tm_json_w_handler_t, int);
int tm_json_write_number (tm_json_w_handler_t, double);
int tm_json_write_int(tm_json_w_handler_t, int);
int tm_json_write_uint(tm_json_w_handler_t, uint32_t);
int tm_json_write_int64(tm_json_w_handler_t, int64_t);
int tm_json_write_uint64(tm_json_w_handler_t, uint64_t);
int tm_json_write_null (tm_json_w_handler_t);
int tm_json_write_object_start (tm_json_w_handler_t);
int tm_json_write_object_end (tm_json_w_handler_t);
int tm_json_write_array_start (tm_json_w_handler_t);
int tm_json_write_array_end(tm_json_w_handler_t);
int tm_json_write_flush(tm_json_w_handler_t);
const char* tm_json_write_result (tm_json_w_handler_t);
size_t tm_json_write_result_length(tm_json_w_handler_t);
int tm_json_write_destroy(tm_json_w_handler_t);

/* Guard to make sure that this is used as C */
#ifdef __cplusplus
}
#endif
